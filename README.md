...
- Next 는 일반적으로 Vercel, AWS 등 에 배포함
- 참고사항 : AWS 는 깡통 PC를 제공함
...
- `/src/app/page.module.css` :
  ...
  - Next 는 기본적으로 moudle css 가 기본 형식
...
- 클라이언트 즉, 웹 브라우저에서 JS 번들로 실행되는 컴포넌트를 Client Component 라고한다.
- 서버 즉, PC 에서 미리 생성해 주는 컴포넌트를 Server Conponent 라고 한다.

### 4.1. 구분해 보자

- 기존의 React 는 html 을 미리 생성하지 않습니다.(웹 브라우저가 HTML 생성함)
- Next 는 Server 에서 미리 html 을 만든다. (만들어진 HTML 출력시킨후 다시 JS 번들을 전달함 > 기능 작동함 (Hydration))
- Next 는 프리랜더링 단계로 HTML 즉 내용을 미리 만들어서 보여주기 때문에 FCP 가 빠르다.

### 4.2. 컴포넌트 구별

- 서버에서 만들어지는 컴포넌트를 Server Component 라고함
- 클라이언트에서 만들어지는 컴포넌트를 Client Component 라고함
...
- Next 는 Server Component 가 기준입니다.
- 만약, `use 훅`들을 사용해야하는 경우는 'use client'를 적어주어야함.
- 만약, `이벤트 핸들러` 들을 사용해야 하는 경우 'use client' 명시해야한다.
- 만약, 웹브라우저에서만 실행되는 라이브러리라면 'use client' 명시해야한다.
- 원칙은 Client Component 에 Server Component 를 import 하지 않도록 하자.
- Client Component 에 Server Component 를 import하면 Client Component 가 됨.
...
function page() {
  ...
  console.log(supakey);
  useEffect(() => {
    console.log("컴포넌트가 웹 브라우저에 보이면 실행하라.");
  }, []);
  return <div className={styles.page}>page</div>;
}
...
- Server Component가 자동으로 Client Component로 변환
...
- children 으로 권장함.(Server Component 를 Client Component 에 사용한다면.)
...
- 타입 스크립트 프로젝트에서 css 또는 module.css 확장자의 타입 정의 필요시
- `src/types 폴더` 생성
- `src/types/css.d.ts` 파일 생성(.d.ts는 타입정의 내용작성)
...
- `/src/components/SearchBar.module.css 파일 생성`
...
- `src/components/SerchBar.tsx` 업데이트
...
- `/src/app/good/[id]/page.module.css` 파일생성
...
- `/src/app/(with-search)/search/page.module.css` 파일생성
...
- `/src/app/(with-search)/search/not-found.tsx` 파일생성

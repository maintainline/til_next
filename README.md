...
- 서버 컴포넌트인 경우
  ...
  - 동적함수(쿠키, 헤더, 쿼리스트링 등..)을 사용하는 컴포넌트가 있을 때

## 2. 빌드 시 정적페이지로 설정되는 기준
...
- `동적함수, 데이터 캐시를 적용하면 Static 으로 빌드됨`
...
```tsx
export const dynamic = "auto"; // 기본값, 아무것도 강제하지않음
export const dynamic = "force-dynamic"; // 강제로 Dynamic 페이지로 설정
export const dynamic = "force-static"; // 강제로 statc 페이지로 설정
export const dynamic = "error"; // 강제로 statc 페이지로 설정하고 빌드 오류 발생시킴
```

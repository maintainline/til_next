...
function page() {
  const handleSubmit = (formData: FormData) => {
    // 일반적으로 아래의 내용을 actions 폴더의 외부파일로 추출해서 배치한다.
    "use server";
    // Form 의 데이터를 읽어보자
    const haha = formData.get("haha") as string;
    const kiki = formData.get("kiki") as string;
...
function ReviewForm() {
  ...
  async function createReviewAction(formData: FormData) {
    ...
    // bash -> FormData { content: 'ㅁㅁㅁ', author: 'ㅁㅍㅍㅍ' }
    // 혹시 데이터가 없으면 null 인데 아래처럼 처리가 옳은지?
    // const content = formData.get("content") as string;
    // const author = formData.get("author") as string;
    //아래가 null 도 담을 수 있으므로 as 보다 더 안전한 코드 적용 => ?.toString()
    ...
    try {
      ...
      // 서버에서 출력됨
      console.log("상품리뷰 등록 성공", response);
      const { id } = await response.json();
      console.log("상품 리뷰 등록결과 id : ", id);
    } catch (error) {
...
- `src/components/CatelList.tsx 파일` 생성
...
- `src/components/CatelList.module.css 파일` 생성
...
export async function createReviewAction(formData: FormData) {
  ...
  try {
    ...
    const { id } = await response.json();
    console.log("상품 리뷰 등록결과 id : ", id);
    // 리뷰가 등록이 되었으면 리랜더링으로 출력을 시켜야함.
    // - 서버에서 호출을 해야한다.
    // - 지금까지 가지고있던 cache 를 무효화 해야한다.
    // - /src/app/good/[id]/page.tsx
...
### 6.3. 특정 레이아웃을 적용한 모든 Page를 재호출 하는 방식
...
- 이런 문제는 Client Component로 만들어서 해결함.
...
export default function ReviewForm() {
  // 리액트 19버전에 추가됨. :  server action 의 매개변수 변화
  // state : 리턴값
...
- `/src/actions/deleteAction.ts` 파일 생성
...
export async function deleteAction(_: any, formData: FormData) {
  //   const goodId = formData.get("good") as string;
  const goodId = formData.get("good")?.toString();
  if (goodId) {
  ...
  try {
    ...
    return {
      status: false,
      message: `해당하는 ${goodId}가 삭제에 실패했습니다. 다시 시도해 주세요.`,
    };
  }
}
```

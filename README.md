...
// 기본값은 데이터를 불러들이면 cache 해서 업데이트 자료를 다시 호출하지 않음.
...
- `/` 에 `.env`로 생성한다.
- `.gitignore`에 .env 파일이 등록되었는지 확인

```txt
NEXT_PUBLIC_API_URL=https://fakestoreapi.com1
```
...
- `/src/app/(with-search)/searcch/ page.tsx` 업데이트
...
- 불필요한 데이터 요청에 수를 줄여서 웹 서비스의 성능을 개선 시킴
...
```bash
 GET / 200 in 489ms
 │ GET https://fakestoreapi.com/products?limit=10 200 in 420ms (cache skip)
 │ │ Cache skipped reason: (auto no cache)
 │ GET https://fakestoreapi.com/products?limit=10 200 in 421ms (cache skip)
 │ │ Cache skipped reason: (auto no cache)
```

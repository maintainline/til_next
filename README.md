...
- 라우터에 의한 Page 출력시 시간이 오래걸리는 경우 존재
- Dynamic Page 는 데이터 연동이라서 로딩이 걸림.
- 비동기 페이지에서는 로딩이 걸림
- Next.js 에서는 page 로딩을 처리하는 loading.tsx 가 존재함.
...
- Next.js 는 파일에서 컨벤션에서 소문자를 기준으로함.
...
- laout.tsx 처럼 하위의 라우터 페이지에 모두 출력이 됨.
- 모두 동일한 loading.ts 를 보여주는 어색함..
- http://localhost:3000/search/now/ => 같은 로딩창...
- http://localhost:3000/search/go/ => 같은 로딩창..

### 5.3. async 가 반드시 존재해야 작동됨.

- page 가 비동기라서 async 가 적용되어야 작동함.
...
- 최적화중 Shift Layout 현상 제거
- 최적화중 스켈레톤을 통해서 사용자를 조금 더 기다리게 유도
...
- 적용하기 src/app/(with-search)/page.tsx
